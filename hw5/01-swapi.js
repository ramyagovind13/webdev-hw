...
const mapStarships = (input) => {
  ...
  const starships = input.map(
    (item) =>
      `${item.name}, manufactured by ${item.manufacturer} ${
        item.cost_in_credits != "unknown"
          ? `cost: - ${item.cost_in_credits.toLocaleString()} credits`
          : ""
      } `
  );
  return starships;
...
const filterStarships = (input) => {
  ...
  const res = input.filter((item) => item.passengers < 10 && item.crew > 1);
  return res;
};
...
const reduceStarships = (input) => {
  ...
  const sum = input.reduce(
    (acc, item) =>
      item.cost_in_credits != "unknown"
        ? acc + parseInt(item.cost_in_credits)
        : acc + 0,
    0
  );
  return "The cost of all starships is " + sum.toLocaleString() + "credits";
};
